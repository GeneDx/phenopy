...
class WeightsTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.ages_distribution_file = os.path.join(
            cls.parent_dir, 'data/phenotype_age.tsv'
        )
        cls.hpo_network, alt2prim, disease_records = generate_annotated_hpo_network(
                cls.obo_file, cls.disease_to_phenotype_file,
                ages_distribution_file=cls.ages_distribution_file
        )

    def test_age_weights(self):
        self.assertEqual(hpo_age_to_weight(
            self.hpo_network, 'HP:0001251', 9.0), 1.0
        )
        self.assertAlmostEqual(hpo_age_to_weight(
            self.hpo_network, 'HP:0001251', 5.0), 0.726, 2
        )
    ...
    def test_make_age_distributions(self):
        ...
        for hpid in ages_truth.index:
            self.assertAlmostEqual(
                ages_truth.loc[hpid]['age_dist'].mean(),
                df.loc[hpid]['age_dist'].mean(), 1
            )

    def test_get_truncated_normal(self):
        self.assertAlmostEqual(
            get_truncated_normal(6.0, 1.0, 0.0, 6.0).mean(), 5.20, 2
        )
        self.assertAlmostEqual(
            get_truncated_normal(6.0, 1.0, 0.0, 6.0).cdf(3.0), 0.0026, 2
        )
        self.assertAlmostEqual(
            get_truncated_normal(6.0, 1.0, 0.0, 6.0).cdf(12.0), 1.0, 2
        )
