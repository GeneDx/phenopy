...
def read_hpo_annotation_file(
    phenotype_annotations_file: str, hpo_network: nx.MultiDiGraph, logger=None
) -> List:
    """
    ...
    try:
        ...
        hpoa_file_error_msg = (
            f"{phenotype_annotations_file} " f"not found or incorrect permissions"
        )
...
def read_custom_annotation_file(
    custom_annotation_file_path: str, hpo_network: nx.MultiDiGraph, logger: None = None
) -> List:
    try:
        ...
        hpoa_file_error_msg = (
            f"{custom_annotation_file_path} " f"not found or incorrect permissions"
        )
        if logger is not None:
            logger.critical(hpoa_file_error_msg)
        else:
...
def load(
    ...
    for disease_accession, disease in disease_to_phenotypes.items():
        ...
        for term_id in disease["terms"]:
            ...
            frequency_weight = phenotype_to_diseases[term_id][disease_accession][
                "frequency"
            ]
            #
...
def frequency_converter(hpoa_frequency: str, default_frequency: float = 0.5) -> float:
    """
